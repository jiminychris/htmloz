function Parser(tileSize) {
  ...
  this.parse = function(mapData, fun) {
    if (!typeof fun === 'function')
      throw new TypeError();
    ...
    $.getJSON(tilesFile, function(defData) {
      imgpreload('resources/images/'+defData.file, function(images) {
        ...
        res.defaultColors = DEFAULT_COLORS;
        res.tilesFile = mapData.tiles_file;
        
        res.tileSheet = tileSheet;
        res.palette = mapData.palette;
        res.tileDefs = tileDefs;
        res.tiles = mapData.tiles.map(function(row, j) {
        ...
        res.imgDataCache = imgDataCache;
        return fun(res);
      });
    });
  }
  
  this.parseFile = function(fname, fun) {
    if (!typeof fun === 'function')
      throw new TypeError();
      
    $.getJSON(fname, function(mapData) {
      parser.parse(mapData, function(res) {
        return fun(res);
      });
    });
  };
